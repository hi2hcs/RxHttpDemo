...
open class ResponseParser<T> : AbstractParser<T> {
    ...
    override fun onParse(response: okhttp3.Response): T {
        ///全部先按String类型处理
        val type: Type = ParameterizedTypeImpl[Response::class.java, String::class.java] //获取泛型类
        var data: Response<String>? = response.convert(type)
        var t: T? = null; //获取data字段
        if ((data?.code ?: -1) == 0) {//成功
            t = GsonUtil.getObject((data?.data ?: ""), mType);
        }
        ...
        if (data?.code != 0 || t == null) { //code不等于0，说明数据不正确，抛出异常
           var msg= data?.msg?:(data?.message?:"")
            throw ParseException(data?.code?.toString() ?: "-1", msg, response)
        }
        return t
    }
}